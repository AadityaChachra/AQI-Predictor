...
for day, group in data.groupby('day'):
    daily_actual_aqi = group['actual_aqi'].mean()

    daily_predicted_aqi = np.mean([
        calculate_overall_aqi(dict(zip(features, predicted_values[i])), breakpoints) for i in group.index
    ])

    
    daily_accuracy_data.append({
        'day': day,
        'daily_actual_aqi': daily_actual_aqi,
        'daily_predicted_aqi': daily_predicted_aqi,
        'accuracy': 100 - abs((daily_predicted_aqi - daily_actual_aqi) / daily_actual_aqi * 100)
    })


daily_accuracy_df = pd.DataFrame(daily_accuracy_data)
daily_accuracy_df.to_csv('Data/FinalData/aqiDailyAccuracy.csv', index=False)

print("Daily accuracy data saved to 'Data/FinalData/aqiDailyAccuracy.csv'.")
