...
data = pd.read_csv('Data/FinalData/aqiTestingData.csv')
...
interpreter, input_details, output_details = load_model('FinalModel/aqiPredictionModel.tflite')
...
test_features = pd.read_csv('Data/FinalData/data.csv')
...
hourly_accuracy_data = []   

for hour, group in data.groupby('hour'):
    hourly_actual_aqi = group['actual_aqi'].mean()

    hourly_predicted_aqi = np.mean([
        calculate_overall_aqi(dict(zip(features, predicted_values[i])), breakpoints) for i in group.index
    ])

   
    hourly_accuracy_data.append({
        'hour': hour,
        'hourly_actual_aqi': hourly_actual_aqi,
        'hourly_predicted_aqi': hourly_predicted_aqi,
        'accuracy': 100 - abs((hourly_predicted_aqi - hourly_actual_aqi) / hourly_actual_aqi * 100)
    })

hourly_accuracy_df = pd.DataFrame(hourly_accuracy_data)
hourly_accuracy_df.to_csv('Data/FinalData/aqiHourlyAccuracy.csv', index=False)

print("Hourly accuracy data saved to 'Data/FinalData/aqiHourlyAccuracy.csv'.")
